...
export async function finalizeOnboardingWizard(options) {
    ...
    if (process.platform === "linux" && !systemdAvailable) {
        await prompter.note("Systemd user services are unavailable. Skipping lingering checks and daemon install.", "Systemd");
    }
    if (process.platform === "linux" && systemdAvailable) {
    ...
    if (process.platform === "linux" && !systemdAvailable && installDaemon) {
        await prompter.note("Systemd user services are unavailable; skipping daemon install. Use your container supervisor or `docker compose up -d`.", "Gateway daemon");
        installDaemon = false;
    }
    if (installDaemon) {
        ...
        if (flow === "quickstart") {
            await prompter.note("QuickStart uses Node for the Gateway daemon (stable + supported).", "Gateway daemon runtime");
        }
        ...
        if (loaded) {
            ...
            if (action === "restart") {
                await withWizardProgress("Gateway daemon", { doneMessage: "Gateway daemon restarted." }, async (progress) => {
                ...
                await withWizardProgress("Gateway daemon", { doneMessage: "Gateway daemon uninstalled." }, async (progress) => {
