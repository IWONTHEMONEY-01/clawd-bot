...
import { addAllowlistEntry, matchAllowlist, maxAsk, minSecurity, recordAllowlistUse, resolveCommandResolution, resolveExecApprovals, } from "../infra/exec-approvals.js";
...
export function createExecTool(defaults) {
    ...
    return {
        ...
        execute: async (_toolCallId, args, signal, onUpdate) => {
            ...
            const elevatedDefaultOn = elevatedDefaults?.defaultLevel === "on" &&
                elevatedDefaults.enabled &&
                elevatedDefaults.allowed;
            const elevatedRequested = typeof params.elevated === "boolean" ? params.elevated : elevatedDefaultOn;
            if (elevatedRequested) {
                ...
                logInfo(`exec: elevated command (${sessionId.slice(0, 8)}) ${truncateMiddle(params.command, 120)}`);
            }
            ...
            if (host === "node") {
                const approvals = resolveExecApprovals(defaults?.agentId, host === "node" ? { security: "allowlist" } : undefined);
                ...
                if (requiresAsk) {
                    const decisionResult = (await callGatewayTool("exec.approval.request", { timeoutMs: 130_000 }, {
                ...
                const raw = (await callGatewayTool("node.invoke", {}, invokeParams));
            ...
            if (host === "gateway") {
                ...
                const allowlistMatch = hostSecurity === "allowlist" ? matchAllowlist(approvals.allowlist, resolution) : null;
                const requiresAsk = hostAsk === "always" ||
                ...
                if (requiresAsk) {
                    ...
                    if (decision === "allow-always") {
                        ...
                        if (hostSecurity === "allowlist") {
                            const pattern = resolution?.resolvedPath ??
                ...
                if (allowlistMatch) {
                    recordAllowlistUse(approvals.file, defaults?.agentId, allowlistMatch, params.command, resolution?.resolvedPath);
                }
            }
