...
export async function updateCommand(opts) {
    ...
    if (!gitCheckout) {
        ...
        const needsConfirm = currentVersion != null && (targetVersion == null || (cmp != null && cmp > 0));
    ...
    if (result.status === "skipped") {
        ...
        if (result.reason === "not-git-install") {
            defaultRuntime.log(theme.warn(`Skipped: this Clawdbot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("clawdbot doctor")}\` and \`${formatCliCommand("clawdbot daemon restart")}\`.`));
    ...
    if (opts.restart) {
        ...
        try {
            ...
            if (!opts.json) {
                ...
                defaultRuntime.log(theme.muted(`You may need to restart the daemon manually: ${formatCliCommand("clawdbot daemon restart")}`));
            }
        }
    }
    else if (!opts.json) {
        ...
        if (result.mode === "npm" || result.mode === "pnpm") {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("clawdbot doctor")}\`, then \`${formatCliCommand("clawdbot daemon restart")}\` to apply updates to a running gateway.`));
        }
        else {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("clawdbot daemon restart")}\` to apply updates to a running gateway.`));
        }
    }
    if (!opts.json) {
