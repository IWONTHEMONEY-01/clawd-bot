import { buildChannelKeyCandidates, resolveChannelEntryMatchWithFallback, } from "../../channels/channel-config.js";
...
function resolveDiscordChannelConfigEntry(entry, matchKey, matchSource) {
    const resolved = {
...
export function resolveDiscordChannelConfigWithFallback(params) {
    ...
    if (match.entry && match.matchKey && match.matchSource) {
        return resolveDiscordChannelConfigEntry(match.entry, match.matchKey, match.matchSource === "parent" ? "parent" : "direct");
    }
    return { allowed: false };
}
