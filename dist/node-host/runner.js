...
import { addAllowlistEntry, matchAllowlist, normalizeExecApprovals, recordAllowlistUse, resolveCommandResolution, resolveExecApprovals, ensureExecApprovals, readExecApprovalsSnapshot, resolveExecApprovalsSocketPath, saveExecApprovals, } from "../infra/exec-approvals.js";
...
async function handleInvoke(frame, client, skillBins) {
    ...
    const allowlistMatch = security === "allowlist" ? matchAllowlist(approvals.allowlist, resolution) : null;
    const bins = autoAllowSkills ? await skillBins.current() : new Set();
    const skillAllow = autoAllowSkills && resolution?.executableName ? bins.has(resolution.executableName) : false;
    ...
    await sendNodeEvent(client, "exec.started", buildExecEventPayload({
        sessionKey,
        runId,
        host: "node",
        command: cmdText,
